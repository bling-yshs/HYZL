...
func DownloadYunzaiMenu() {
	...
	if err == nil {
		print_utils.PrintWithEmptyLine("检测到当前目录下已存在 " + yunzai.GetYunzai().Name + "，请问是否需要重新下载？(是:y 返回菜单:n)")
		userChoice := input_utils.ReadChoice([]string{"y", "n"})
		if userChoice == "y" {
			print_utils.PrintWithColor(ct.Red, true,
				"重新下载云崽会移除当前目录下的 "+yunzai.GetYunzai().Name+" 文件夹，云崽的数据将会被全部删除，且不可恢复，请再次确认是否继续？(是:y 返回菜单:n)")
			userChoice := input_utils.ReadChoice([]string{"y", "n"})
			if userChoice == "n" {
				return
			}
			//删除文件夹
			print_utils.PrintWithEmptyLine("正在删除 " + yunzai.GetYunzai().Name + " 文件夹...")
			os.RemoveAll(yunzai.GetYunzai().Path)
		}
		if userChoice == "n" {
			return
		}
	...
	_, err = os.Stat(path.Join(yunzai.GetYunzai().Path, "node_modules/icqq/package.json"))
